...
import { Instagram, Twitter, Linkedin, Github, Mail, ArrowUpRight, Facebook, MessageCircle } from "lucide-react";
...
const Footer = () => {
  ...
  const socialLinks = [
    {
      ...
      colorClass: "hover:bg-[#E4405F]/10 hover:text-[#E4405F] hover:border-[#E4405F]/30"
    },
    {
  ...
  return (
    <footer className="bg-background text-foreground pt-16 md:pt-24 pb-10 px-6 border-t border-white/[0.03] relative overflow-hidden">
        ...
        <div className="flex flex-wrap justify-center gap-3 md:gap-4 mb-4 md:mb-6">
          {socialLinks.map((social, index) => (
            <a
              ...
              className={`w-10 h-10 md:w-12 md:h-12 flex items-center justify-center bg-white/[0.02] border border-white/[0.05] rounded-full text-muted-foreground transition-all duration-500 hover:-translate-y-1 ${social.colorClass}`}
            >
            ...
            <div className="flex gap-6">
              {legalLinks.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="text-muted-foreground hover:text-foreground text-[10px] font-bold uppercase tracking-[0.2em] transition-colors"
                >
                  {item.name}
                </Link>
              ))}
